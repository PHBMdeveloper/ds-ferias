...
## 2026-05-15 - [XSS em Mensagens de Feedback]
**Vulnerabilidade:** A API de feedback não realizava a sanitização completa das mensagens e nomes anônimos enviados pelo usuário, removendo as tags ao invés de escapar as entidades HTML. Isso permitia que um atacante contornasse a sanitização superficial e realizasse injeção de script persistente (XSS) ao fornecer entradas maliciosas em relatórios.
**Learning:** O método de limpeza de texto estava suscetível a ataques persistentes, já que a remoção cega não evita todas as variações de codificação e ofuscação.
**Prevention:** Utilizar a substituição rigorosa de entidades HTML para desarmar códigos em vez de tentar remover strings com expressões regulares.
