...
**Vulnerabilidade:** A rota de criação de feedback (`app/api/feedback/route.ts`) recebia os campos \`message\` e \`anonymousName\` diretamente do corpo da requisição e os persistia no banco de dados sem sanitização.
**Learning:** Dados controlados pelo usuário estavam sendo armazenados no banco e renderizados na visualização administrativa sem proteção contra Cross-Site Scripting (XSS) persistente.
**Prevention:** Sempre aplicar sanitização (como \`sanitizeText\`) em textos providos pelo usuário antes de salvá-los no banco ou incluí-los em logs.
