...
**Vulnerabilidade:** A chamada `prisma.user.update` no endpoint de backoffice (`/api/users/[id]`) não possuía um bloco `select`, fazendo com que o Prisma retornasse todas as colunas da tabela, incluindo `passwordHash`. Como a resposta retornava o objeto do Prisma não modificado, isso expunha o hash da senha pela API para os administradores/RH.
**Learning:** Operações `create` e `update` do Prisma retornam todos os campos do modelo por padrão. Dados sensíveis (ex: `passwordHash`) devem ser sempre explicitamente excluídos usando um bloco `select` (ou omit) em handlers de API para prevenir o vazamento de dados.
**Prevention:** Sempre revisar os retornos de endpoints que serializam objetos vindos diretamente do Prisma e utilizar explicitamente a propriedade `select` para definir quais campos podem trafegar para o cliente de forma segura.
