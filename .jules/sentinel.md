...
## 2026-05-15 - [Stored XSS & Log Injection em Inputs de Texto (Feedback)]
**Vulnerabilidade:** A rota `/api/feedback` aceitava inputs de texto diretos do usuário (`message` e `anonymousName`) sem sanitização, o que poderia permitir ataques de XSS armazenado (Stored XSS) no Backoffice ou ataques de injeção de log se o atacante enviasse conteúdo malicioso com quebras de linha ou caracteres de controle ou tags HTML.
**Learning:** Dados controlados pelo usuário (User-controlled data) nunca devem ser aceitos crus em logs e muito menos persistidos diretamente no banco de dados quando essas informações vão ser renderizadas na interface ou processadas por pipelines de logging. O `sanitizeText` da `lib/validation.ts` deve ser padrão na leitura do body.
**Prevention:** Sempre invocar utilitários de sanitização como o `sanitizeText` nos parâmetros de entrada durante o parse dos requests das rotas de API, antes de validá-los como vazios, antes de logá-los, e antes de persisti-los no banco.
