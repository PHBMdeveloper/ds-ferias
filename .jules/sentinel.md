...
## 2026-05-15 - [Log Injection & XSS Prevented in API Routes]
**Vulnerability:** User-controlled fields in `/api/feedback` (`type`, `message`, `anonymousName`) and `/api/blackout-periods` (`reason`, `department`) were logged and persisted to the database without sanitization, allowing potential Log Injection and Stored XSS.
**Learning:** Even internal backend logs or admin-only views can be vectors if input isn't sanitized. Extracting fields and sanitizing them explicitly before any usage is critical.
**Prevention:** Always apply `sanitizeText` to string fields extracted from `request.json()` before logging or using them in Prisma `create`/`update` statements.
