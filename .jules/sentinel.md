...
## 2025-05-13 - [Fix Log Injection and Stored XSS in Feedback Route]
**Vulnerability:** The feedback route accepted raw JSON body inputs (`message`, `anonymousName`, `type`) and directly wrote them to the database and structured logs (`logger.warn`) without sanitization, creating Stored XSS and Log Injection risks. Additionally, passing an unsanitized metadata object containing a `message` key (e.g., `{ message: userMessage }`) to `logger.warn("Primary string", { message: userMessage })` caused the logger to overwrite the primary log string with the user-provided payload.
**Learning:** The structured logger configuration explicitly overrides the primary `message` parameter if a `message` key exists in the metadata object. This is a critical risk for Log Forging and general observability.
**Prevention:** Always use `sanitizeText` for user-supplied strings before logging or persisting. Furthermore, always rename payload fields named `message` when logging them in metadata objects (e.g., use `inputMessage` or `userMessage` instead of `message`).
