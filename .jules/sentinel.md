...
## 2026-05-15 - [Stored XSS and Log Injection in Feedback API]
**Vulnerability:** User-controlled inputs (`type`, `message`, `anonymousName`) were directly persisted to the database and logged without sanitization. This allows an attacker to inject HTML/JS payloads (Stored XSS) when admins view the feedback, or manipulate log files (Log Injection) by inserting newline characters or spoofed log entries.
**Learning:** Even internal-facing API endpoints must sanitize all inputs before usage. React's default escaping mitigates some XSS risks, but logs and third-party tools processing logs are vulnerable to injection attacks if inputs are not scrubbed.
**Prevention:** Always use the centralized `sanitizeText()` utility from `lib/validation.ts` to strip HTML tags and sanitize inputs before logging and persisting to the database.
