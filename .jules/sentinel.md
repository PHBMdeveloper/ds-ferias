...
## 2024-05-05 - Prisma Update Queries Returning Sensitive Fields
**Vulnerability:** Prisma's `update` and `create` operations return all model fields by default if no `select` clause is provided. In `app/api/users/[id]/route.ts`, the user object returned from `tx.user.update` lacked a `select` block, exposing the `passwordHash` field in the HTTP JSON response.
**Learning:** Default object hydration in ORMs like Prisma poses an inherent risk of Excessive Data Exposure. Developers must explicitly map the response footprint on every data modification endpoint, especially for User models or models with tokens/secrets. The absence of a strict select block is an anti-pattern.
**Prevention:** Always enforce a strict `select` payload containing only safe, non-sensitive fields when returning entities directly from database modifications to the client payload. Use API contract testing or strict TypeScript mapping to verify response boundaries and catch leaking secrets.
