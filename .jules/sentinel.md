...
## 2026-05-15 - [Stored XSS in Feedback Endpoint]
**Vulnerability:** The `/api/feedback` endpoint accepted and stored the `message` and `anonymousName` fields directly into the database without sanitization, creating a Stored XSS vector when an admin later views the feedbacks.
**Learning:** Even internal tool feedback systems must strictly sanitize user inputs. Using the existing `sanitizeText` utility prevents injected `<script>` or HTML tags from persisting.
**Prevention:** Apply input sanitization at the API boundary before passing user-controlled strings to the database layer (Prisma).
