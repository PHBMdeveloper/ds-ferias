...
describe("dashboardDataService", () => {
  beforeEach(() => {
    ...
    mockFindUserWithBalance.mockResolvedValue(null);
    mockFindUserDepartment.mockResolvedValue(null);
    mockSyncAcquisitionPeriodsForUser.mockResolvedValue([]);
    mockFindAcquisitionPeriodsForUser.mockResolvedValue([]);
  ...
  describe("getUserAcquisitionPeriods", () => {
    it("syncs acquisition periods and returns acquisition periods list", async () => {
      ...
      mockFindUserWithBalance.mockResolvedValue({ hireDate } as any);
      mockFindAcquisitionPeriodsForUser.mockResolvedValueOnce([]); // needsSync = true (empty)
      mockSyncAcquisitionPeriodsForUser.mockResolvedValue([{ id: "p1" }] as any);
      mockFindAcquisitionPeriodsForUser.mockResolvedValueOnce([{ id: "p1" }] as any); // after sync
    ...
    it("returns existing periods when sync is not needed", async () => {
      const hireDate = new Date("2023-01-01T00:00:00.000Z");
      const periods = [{ id: "p1", endDate: new Date("2028-01-01") }];
      mockFindUserWithBalance.mockResolvedValue({ hireDate } as any);
      mockFindAcquisitionPeriodsForUser.mockResolvedValue(periods as any);

      const out = await getUserAcquisitionPeriods("u1");
      expect(mockSyncAcquisitionPeriodsForUser).not.toHaveBeenCalled();
  ...
  describe("entitlement and sidebar context", () => {
    ...
    it("getMyVacationSidebarContext returns concessive context when hireDate exists", async () => {
      ...
      mockFindUserWithBalance.mockResolvedValue({
        hireDate,
        department: "Produto",
        vacationRequests: [],
      });
      ...
      // Needs sync because end date is in the past
      mockFindAcquisitionPeriodsForUser.mockResolvedValueOnce(periods as any);
      mockSyncAcquisitionPeriodsForUser.mockResolvedValue(periods as any);
      mockPrismaVacationFindMany.mockResolvedValue([
