...
- Cobertura de statements: **91.00%** (Meta alcançada: alvo >= 90%).
...
- **Analytics Estratégico:** Validação da lógica de disponibilidade e filtros dinâmicos na visão de Times.
- **Sincronização de períodos aquisitivos:** `acquisitionRepository.ts` (98%+).
- **Segurança Sentinel:** Testes de sanitização CSV e fluxos de troca de senha obrigatória.
